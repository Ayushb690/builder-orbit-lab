...
export default function Dashboard() {
  ...
  const getOverallStats = () => {
    ...
    const extra = todayRecords.filter(
      (record) => record.status === "extra",
    ).length;
    const cancelled = todayRecords.filter(
    ...
    const attendancePercentage =
      subjects.length > 0 ? Math.round((present / subjects.length) * 100) : 0;
  ...
  const getTotalStats = () => {
    ...
    const overallAttendance =
      totalClasses > 0
        ? Math.round(((present + extra) / totalClasses) * 100)
        : 0;
              ...
              <div key={subject.id} className="p-4 rounded-lg border bg-card">
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  {todayAttendance[subject.id] && (
                    <Badge
                      className={`${
                        todayAttendance[subject.id] === "present"
                          ? "bg-success"
                          : todayAttendance[subject.id] === "absent"
                            ? "bg-destructive"
                            : todayAttendance[subject.id] === "extra"
                              ? "bg-info"
                              : "bg-muted"
                      } text-white`}
                    >
