...
export default function AttendanceChatbot() {
  ...
  const getStudentData = () => {
    ...
    const records: AttendanceRecord[] = savedAttendance ? JSON.parse(savedAttendance) : [];
  ...
  const generateBotResponse = (userMessage: string): string => {
    ...
    const attendanceRate = totalClasses > 0 ? Math.round(((totalPresent + totalExtra * 2) / (totalPresent + totalAbsent + totalExtra * 2)) * 100) : 0;
    ...
    const subjectStats = subjects.map(subject => {
      ...
      return {
        ...
        extra,
        remaining,
        rate: (present + absent + extra) > 0 ? Math.round(((present + extra * 2) / (present + absent + extra * 2)) * 100) : 0
      };
    });
    ...
    if (lowerMessage.includes('subject') || lowerMessage.includes('performance')) {
      if (subjectStats.length === 0) return "You haven't set up any subjects yet. Go to setup to add your subjects!";
    ...
    if (lowerMessage.includes('goal') || lowerMessage.includes('target')) {
      const classesNeeded = subjectStats.reduce((sum, s) => sum + s.remaining, 0);
  ...
  const initializeChat = () => {
    if (messages.length === 0) {
      const welcomeMessage: ChatMessage = {
        id: 'welcome',
        type: 'bot',
        message: '👋 Hi! I\'m your attendance assistant. I can provide insights about your attendance data, performance analysis, and help you track your goals. What would you like to know?',
        timestamp: new Date()
      };
